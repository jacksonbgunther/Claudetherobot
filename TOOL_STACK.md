...
| **Google Calendar** | Content calendar / posting schedule visibility | Yes, connected | Yes | Already connected | $0 | — | Useful, not blocking | — | 🔴 **broken** — `list_calendars` failed 2026-08-19: OAuth token expired, needs re-authorization. Human-only fix (claude.ai connector settings); Claude cannot run an OAuth flow from inside a session. |

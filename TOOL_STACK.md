...
Last updated: 2026-08-24 (Day 5, full loop — GitHub/Gmail/Drive
re-verified live; Threads endpoint re-tried, still unverified).
...
| **Threads API, direct** | Publish to Threads — also not covered by Buffer here | Yes, Meta Graph API (container create + publish) | Yes | Meta Developer account + App with Threads API use case, human added as a "tester" (skips App Review for single-account use) | $0 — no pricing tier exists | Yes, fully free | Yes, for Threads publishing specifically | Buffer, if Threads ever gets linked there instead | 🔴 **credentials set, still unverified** — `THREADS_ACCESS_TOKEN`/`THREADS_USER_ID` present since 2026-08-23. First check hit `403` (`connect_rejected`) at this session's network proxy. Re-tried 2026-08-24: this time a flat connection failure (curl exit 56, no HTTP status at all) — a different failure signature, not a confirmed fix, so still recorded as unverified rather than read as progress. Needs `graph.threads.net` added to this environment's network egress allowlist before the token itself can be confirmed live. |
...
| **Gmail** | Individual, low-volume outreach (Const. §59-61 policy) | Yes, connected | Yes, within the no-mass-outreach policy | Already connected | $0 | — | Yes | — | 🟢 verified 2026-08-24 (`list_labels` call succeeded, real inbox data returned) |
| **Google Calendar** | Content calendar / posting schedule visibility | Yes, connected | Yes | Already connected | $0 | — | Useful, not blocking | — | 🔴 **broken** — failed again 2026-08-24 (`requires re-authorization (token expired)`), same failure as every check since 2026-08-19. Human-only fix (claude.ai connector settings); Claude cannot run an OAuth flow from inside a session. Not blocking — nothing in the loop depends on it yet. |
| **Google Drive** | Asset handoff/storage between me and my human | Yes, connected | Yes | Already connected | $0 | — | Useful, not blocking | — | 🟢 verified 2026-08-24 — `ClaudeTheRobot` asset folder still found: https://drive.google.com/drive/folders/177e_eevOEvLTvN-G-9HLdx8EuPfyJQ1n |
| **GitHub** | Approval workflow, decision audit trail, repo | Yes, connected | Yes | Already connected | $0 | — | Yes | — | 🟢 verified 2026-08-24 — open-issues list retrieved successfully (#1-#5) |
...
|---|---|---|---|---|---|---|---|---|---|
| **Image generation** (Gemini image models — the old Imagen 3 `predict` endpoint is gone, see below) | Thumbnails, quote cards, carousel visuals — the cheapest real path to visual content | Yes, standard REST API | Yes, once billing is enabled (see status) | A Gemini/Google AI Studio API key (free to create; billing is pay-per-image) | ~$0.02-0.04/image | Free tier exists for the Gemini API generally; image *generation* specifically has a **0 free-tier quota** — see status | Yes, for any visual content, and it's the leanest option available | Flux 2 Pro (~$0.02/unit) is comparable; Higgsfield is *not* cheaper — see below | 🟡 **downgraded 2026-08-24 — key present, generation blocked, not "verified live."** The 2026-08-23 status above was wrong to call this verified: `GET /v1beta/models` (a free metadata call) was tested, actual generation never was. Real test today (issue #7's ask for honest quality output before committing to an emotional niche): the old `imagen-3.0-generate-002:predict` endpoint from `generate-image/SKILL.md` is `404` — that model/endpoint no longer exists. Current image-capable models are `gemini-2.5-flash-image`, `gemini-3-pro-image(-preview)`, `gemini-3.1-flash-image(-preview/-lite)`, called via `generateContent`, not `predict`. Tried `gemini-2.5-flash-image:generateContent` — got `429 RESOURCE_EXHAUSTED`, `generate_content_free_tier_requests` **limit: 0** for that model. This is a billing-not-enabled problem, not a code/model-name problem alone (even the right endpoint has zero free quota) — human needs to enable billing on the Google Cloud project behind this API key before any image can actually be generated. Filed as a `human-manual` approval. `generate-image/SKILL.md` also needs its endpoint/model rewritten once billing is confirmed. Closes issue #2's Gemini ask only partially — key exists, generation doesn't work yet. |
| **Shotstack** | JSON-driven video assembly — captions + images + audio into a short video, without needing full generative video | Yes, REST API, sandbox for testing | Yes | Free signup, no card required for sandbox | $0.30/min pay-as-you-go (or $39/mo plan at $0.20/min — skip the plan, PAYG fits better at this scale) | 10 free credits/30 days | Yes, if pursuing narrated/edited short-form video — this plus cheap images plus ElevenLabs covers a real video pipeline for a few dollars, not a few hundred | Full generative video (Higgsfield/Runway) — see why that's Tier 3, not this tier | 🟢 **verified live 2026-08-23** — `SHOTSTACK_API_KEY` set, confirmed with a real `GET /stage/templates` call (`200 OK`, real account response, no cost). No template/render built yet. |
| **ElevenLabs (free tier)** | Voiceover for narrated content | Yes, REST API | Yes | Free signup | $0 to start | Yes — 10,000 credits/mo ≈ 10 min TTS | Only if narrated video is the chosen format — not necessary for text/image-first content | — | 🟡 **key present but narrowly scoped 2026-08-23** — `ELEVENLABS_API_KEY` set, and both `GET /v1/user` and `GET /v1/models` returned `401` with `missing_permissions` (`user_read`, `models_read` respectively) rather than an auth failure — the key itself authenticates, it's just not scoped for account/model reads. Likely scoped to TTS generation only; unconfirmed until an actual generation call is needed (that would spend usage, so not tested here). Not blocking anything yet — no narrated content in the queue. |
